...
#Original resolution is Nx = 256, Nz = 32
function HorizontalConvectionSimulation(; Ra=1e12, h₀_frac=0.6, Nx=324, Ny=1, Nz=48, output_writer=true, advection=true, architecture=CPU())
    
    ## Constant parameters and functions
    H = 2.0            # vertical domain extent
    Lx = 8H            # horizontal domain extent

    Ny == 1 ? Ly = 0.0 : Ly = H/4 # meridional domain extent

    seasonal_amplitude = 1.0 #Seasonal Parameters
    seasonal_period = 30.0
    
    Pr = 1.8    # Prandtl number: original value is 1.0 
    ...
    if seasonal_amplitude != 0.0
        seasonal_type = "SeasonalCycle"
    end
    filename_prefix = string(runtype, "_h", h₀_frac, "_Ra", Ra, seasonal_type, seasonal_amplitude)
    ...
    # adding sin function for seasonal buoyancy forcing 
    seasonal_forcing(t) = (1 + seasonal_amplitude * sin(2π * t / seasonal_period))


    # changing buoyancy forcing to tanh 
    ϵ = 1e-6
    β = atanh(1-ϵ)
    α = (2/Lx) * (β-atanh(-1+ϵ))
